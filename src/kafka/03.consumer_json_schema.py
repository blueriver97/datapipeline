...
def consume_json_schema_messages():
    """
    Kafka 토픽에서 JSON Schema 메시지를 소비합니다.
    """
    schema_registry_conf = {"url": KAFKA_CONF["schema_registry_url"]}
    schema_registry_client = SchemaRegistryClient(schema_registry_conf)
    json_schema_deserializer = JSONDeserializer(None, schema_registry_client=schema_registry_client)
    ...
    try:
        while True:
            ...
            if msg.error():
                ...
                # 정상 메시지 처리
                message = json_schema_deserializer(msg.value(), SerializationContext(msg.topic(), MessageField.VALUE))

...
def produce_json_schema_gsr_message():
    """
    AWS Glue Schema Registry를 사용하여 JSON Schema 메시지를 Kafka 토픽으로 전송합니다.
    """
    session = boto3.Session(profile_name=AWS_CONF["profile_name"], region_name=AWS_CONF["region_name"])
    glue_client = session.client("glue")
    ...
    producer.produce(
        ...
        value=serializer.serialize(
            ...
            data_and_schema=(
                value,
                JsonSchema(json.dumps(Value.model_json_schema())),
            ),
        ),
...
if __name__ == "__main__":
    ...
    produce_json_schema_gsr_message()

...
from aws_schema_registry.serde import KafkaDeserializer
...
TOPIC_NAME = config["topics"]["names"]["json_schema_gsr"]
...
def consume_json_schema_gsr_messages():
    """
    AWS Glue Schema Registry를 사용하여 JSON Schema 메시지를 소비합니다.
    """
    ...
    try:
        while True:
            ...
            if msg.error():
                ...
                # 정상 메시지 처리
                message, schema = deserializer.deserialize(msg.topic(), msg.value())
...
if __name__ == "__main__":
    consume_json_schema_gsr_messages()
